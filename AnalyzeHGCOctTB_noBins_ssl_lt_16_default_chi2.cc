...
void AnalyzeHGCOctTB::EventLoop(const char *data, const char *energy) {
  ...
  sprintf(path_to_save, "Results_new_chi2_noBins_ssl_lt_16_default_chi2/");
 ...
 float ee_rescaling = 1.035;
  ...
  file_ecal_ene_frac.open("Results_new_chi2_noBins_ssl_lt_16_default_chi2/ecal_hcal_Eneupto13X0_true_pred_SS.txt",ios::out);  
  //file_ecal_ene_frac.open("Results_new_chi2_4bins_ssl_lt_16_varCorPi0_fracRawE/fitted_parameters_4bins_ssl_lt_16_varCorPi0_fracRawE.txt",ios::out);
  ...
  float var_pi0_frac_bins[5] = {0.0, 999999999.0, 999999999.0, 999999999.0, 999999999.0};
  ...
  sprintf(name1,"Results_new_chi2_noBins_ssl_lt_16_default_chi2/fitted_parameters_noBins_ssl_lt_16_default_chi2.txt");
  std::fstream params;
...
//  f_w3_bin10_ss->FixParameter(3, param_vector[179]); 
   ...
   for (int pi0Frac_bin=0; pi0Frac_bin<1; pi0Frac_bin++){
     for(int i =0; i<85; i++)
         {
         for(int i = 0; i < 3; i++) {
             for(int j = 0; j < 3; j++) {
  ...
  for (jentry=0; jentry<nentries;jentry++,hgc_jentry++)
  {
    ...
    // ==============print number of events done == == == == == == == =
      ...
      if (rechit_shower_start_layer<=15) 
      {
        for (int ecal_bins=0; ecal_bins<1; ecal_bins++)
  ...
  ////////////////////////////////////////////////////    
   	   ...
   	   for (int ecal_bins=0; ecal_bins<1; ecal_bins++)
   	   { 
   	   sprintf(name,"Results_new_chi2_noBins_ssl_lt_16_default_chi2/chi2_opt_ssInEE_ecal_ene_fracFact_%f_to_%f_flatEn.txt",var_pi0_frac_bins[ecal_bins],var_pi0_frac_bins[ecal_bins+1]);
