...
void AnalyzeHGCOctTB::EventLoop(const char *data, const char *energy) {
 ...
 float ee_rescaling = 1.035;
  ...
  file_ecal_ene_frac.open("Results/ecal_hcal_EneFrac_true_pred_SS.txt",ios::out);  
...
//  float mips_hcal_ene_frac_bins[6] = {0.0, 0.4, 0.8, 1.2, 999999999.0};
  ...
  float ss_ecal_ene_frac_bins[11] = {0.0,0.2,0.4,0.6,0.8, 999999999.0};
  float mips_hcal_ene_frac_bins[6] = {0.0, 0.65, 999999999.0};
  ...
  //sprintf(name1,"Results/fitted_parameters_9ss_4mips_bins.txt");
  sprintf(name1,"Results/fitted_parameters_5ss_2mips_bins_tureE.txt");
...
//  //fit function MipsInEE for 9ss_4mips
//
...
//
////fit function SSInEE for 9ss_4mips
//
...
//
//  TF1* f_w1_bin3_ss = new TF1("f_w1_bin3_ss","sqrt([0]*[0]+[1]/x)+[2]/x+[3]*x",5,360);
...
//
//
//  TF1* f_w1_bin4_ss = new TF1("f_w1_bin4_ss","sqrt([0]*[0]+[1]/x)+[2]/x+[3]*x",5,360);
...
//fit function MipsInEE for 5ss_2mips
...
//fit function SSInEE for 5ss_2mips
  ...
  f_w3_bin5_ss->FixParameter(3, param_vector[119]);
   ...
   for (int ecal_bin=0; ecal_bin<5; ecal_bin++){
     for(int i =0; i<85; i++)
         {
         for(int i = 0; i < 3; i++) {
             for(int j = 0; j < 3; j++) {
 ...
 //        cout<<ss_ecal_ene_frac_bins[ecal_bin]<<endl;
   ...
   for (int hcal_bin=0; hcal_bin<2; hcal_bin++){
     for(int i =0; i<85; i++)
       {
         for(int i = 0; i < 2; i++) {
           for(int j = 0; j < 2; j++) {
  ...
  for (jentry=0; jentry<nentries;jentry++,hgc_jentry++)
  {
    ...
    // ==============print number of events done == == == == == == == =
     ...
     // double ecal_ene_frac = EE_detscale/total_energy;
      ...
      {
        for (int hcal_bins=0; hcal_bins<2; hcal_bins++)
        {//cout<<"ecal_bins="<<ecal_bins<<endl; 
          ...
          if(hcal_bins==0)
            {
            w2 = f_w2_bin1_mips->Eval(trueBeamEnergy); if(w2<0.0) w2=0.0;
            w3 = f_w3_bin1_mips->Eval(trueBeamEnergy); if(w3<0.0) w3=0.0;
  ...
  ////////////////////////////////////////////////////    
    ...
    for (int hcal_bins=0; hcal_bins<2; hcal_bins++)
    { 
      sprintf(name,"Results/chi2_opt_mipsInEE_hcal_ene_fracFact_%f_to_%f_flatEn.txt",mips_hcal_ene_frac_bins[hcal_bins],mips_hcal_ene_frac_bins[hcal_bins+1]);
      ...
      for (int ecal_bins=0; ecal_bins<5; ecal_bins++)
      { 
      sprintf(name,"Results/chi2_opt_ssInEE_ecal_ene_fracFact_%f_to_%f_flatEn.txt",ss_ecal_ene_frac_bins[ecal_bins],ss_ecal_ene_frac_bins[ecal_bins+1]);
      std::ofstream file_bin1;
      file_bin1.open(name,ios::out);
