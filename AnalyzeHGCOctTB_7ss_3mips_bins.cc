...
void AnalyzeHGCOctTB::EventLoop(const char *data, const char *energy) {
  ...
  sprintf(path_to_save, "Results_new_chi2_7ss_3mips_bins/");
 ...
 float ee_rescaling = 1.035;
  ...
  file_ecal_ene_frac.open("Results_new_chi2_7ss_3mips_bins/ecal_hcal_EneFrac_true_pred_SS.txt",ios::out);  
  ...
  //float ss_ecal_ene_frac_bins[11] = {0.0, 0.05, 0.1, 0.18, 0.26, 0.34, 0.42, 0.5, 0.6, 0.75, 1.01};
  //float mips_hcal_ene_frac_bins[6] = {0.0, 0.4, 0.6, 0.75, 0.9, 1.01};
  float ss_ecal_ene_frac_bins[8] = {0.0, 0.1, 0.22, 0.34, 0.46, 0.6, 0.75, 1.01};
  float mips_hcal_ene_frac_bins[4] = {0.0, 0.4, 0.7, 1.01};
  ...
  sprintf(name1,"Results_new_chi2_7ss_3mips_bins/fitted_parameters_10bins.txt");
  std::fstream params;
  ...
  TF1* f_w2_bin3_mips = new TF1("f_w2_bin3_mips","sqrt([0]*[0]+[1]/x)+[2]/x+[3]",5,360);
...
//fit function SSInEE
  ...
  TF1* f_w1_bin5_ss = new TF1("f_w1_bin5_ss","sqrt([0]*[0]+[1]/x)+[2]/x+[3]",5,360);
  ...
  TF1* f_w1_bin9_ss = new TF1("f_w1_bin9_ss","sqrt([0]*[0]+[1]/x)+[2]/x+[3]",5,360);
  ...
  f_w3_bin10_ss->FixParameter(3, param_vector[179]);
   ...
   for (int ecal_bin=0; ecal_bin<7; ecal_bin++){
     for(int i =0; i<85; i++)
         {
         for(int i = 0; i < 3; i++) {
             for(int j = 0; j < 3; j++) {
 ...
 //        cout<<ss_ecal_ene_frac_bins[ecal_bin]<<endl;
     }
   std::vector<ROOT::Math::SVector<double, 2> >consts_mipsInEE_vec;
   std::vector<ROOT::Math::SVector<double, 2> >values_mipsInEE_vec;
   std::vector<ROOT::Math::SMatrix<double,2, 2, ROOT::Math::MatRepStd<double,2>> > coeff_mipsInEE_vec;
   bool isInverted = false;
   ROOT::Math::SMatrix<double,2, 2, ROOT::Math::MatRepStd<double,2> > coeffs_mipsInEE;
   ROOT::Math::SVector<double, 2> consts_mipsInEE;
   ROOT::Math::SVector<double, 2> values_mipsInEE;
 
   std::vector<std::vector<ROOT::Math::SVector<double, 2> > > consts_mipsInEE_vec_diff_bins;
   std::vector<std::vector<ROOT::Math::SVector<double, 2> > > values_mipsInEE_vec_diff_bins;
   std::vector<std::vector<ROOT::Math::SMatrix<double,2, 2, ROOT::Math::MatRepStd<double,2>> > >coeff_mipsInEE_vec_diff_bins;
 
   for (int hcal_bin=0; hcal_bin<3; hcal_bin++){
     for(int i =0; i<85; i++)
       {
         for(int i = 0; i < 2; i++) {
           for(int j = 0; j < 2; j++) {
             coeffs_mipsInEE(i,j) = 0.0;
           }
           consts_mipsInEE(i) = 0.0;
           values_mipsInEE(i) = 0.0;
         }
         consts_mipsInEE_vec.push_back(consts_mipsInEE);
         values_mipsInEE_vec.push_back(values_mipsInEE);
         coeff_mipsInEE_vec.push_back(coeffs_mipsInEE);
       }
       
       consts_mipsInEE_vec_diff_bins.push_back(consts_mipsInEE_vec);
         values_mipsInEE_vec_diff_bins.push_back(values_mipsInEE_vec);
         coeff_mipsInEE_vec_diff_bins.push_back(coeff_mipsInEE_vec);
  ...
  for (jentry=0; jentry<nentries;jentry++,hgc_jentry++)
  {
    ...
    // ==============print number of events done == == == == == == == =
      ...
      //double ss_ecal_ene_frac_bins[6] = {0.0, 0.06, 0.2, 0.4, 0.6, 1.01};

      
      if (rechit_shower_start_layer>28)
      {
        for (int hcal_bins=0; hcal_bins<3; hcal_bins++)
        {//cout<<"ecal_bins="<<ecal_bins<<endl; 
          if(hcal_ene_frac>=mips_hcal_ene_frac_bins[hcal_bins] && hcal_ene_frac<mips_hcal_ene_frac_bins[hcal_bins+1])
          {//cout<<"ecal_ene_frac="<<ecal_ene_frac<<endl;
              for(int i_bin=0;i_bin<85;i_bin++)
              {
                  ...
                  if(trueBeamEnergy>=Elist[i_bin] && trueBeamEnergy <=Elist[i_bin]+4)
                  {
                      ...
                      sigma2 = (0.09*0.09)*(total_energy*total_energy) + (1.23*1.23)*total_energy;
                      ...
                      if(IsIn_en_range)
                      {
                          E_beam = Elist[i_bin]+2;
                              ...
                              if(chi2_method == 0) // method -input to chi2 are in units of MIPs
                                  {
                                  coeffs_mipsInEE(0,0) = (rechitEnergySum_FH*rechitEnergySum_FH)/sigma2;
                                  coeffs_mipsInEE(0,1) = (rechitEnergySum_AH*rechitEnergySum_FH)/sigma2;
                                  coeffs_mipsInEE(1,0) = (rechitEnergySum_FH*rechitEnergySum_AH)/sigma2;
                                  coeffs_mipsInEE(1,1) = (rechitEnergySum_AH*rechitEnergySum_AH)/sigma2;
                                  
                                  consts_mipsInEE(0)   = (E_beam-O)*rechitEnergySum_FH/sigma2;
                                  consts_mipsInEE(1)   = (E_beam-O)*rechitEnergySum_AH/sigma2;

                                  consts_mipsInEE_vec_diff_bins[hcal_bins][i_bin] += consts_mipsInEE;
                                  coeff_mipsInEE_vec_diff_bins[hcal_bins][i_bin] += coeffs_mipsInEE;

                                  }
                              else // method -input to chi2 are in units of GeV -> We use this one
                                  {
                                  coeffs_mipsInEE(0,0) = (FH_detscale*FH_detscale)/sigma2;
                                  coeffs_mipsInEE(0,1) = (AH_detscale*FH_detscale)/sigma2;
                                  coeffs_mipsInEE(1,0) = (FH_detscale*AH_detscale)/sigma2;
                                  coeffs_mipsInEE(1,1) = (AH_detscale*AH_detscale)/sigma2;
                                  
                                  consts_mipsInEE(0)   = (E_beam-O)*FH_detscale/sigma2;
                                  consts_mipsInEE(1)   = (E_beam-O)*AH_detscale/sigma2;
          ...
          if(hcal_bins==0)
            {
            w2 = f_w2_bin1_mips->Eval(trueBeamEnergy);
            w3 = f_w3_bin1_mips->Eval(trueBeamEnergy);
            ...
            {
            w2 = f_w2_bin2_mips->Eval(trueBeamEnergy);
            w3 = f_w3_bin2_mips->Eval(trueBeamEnergy);
            ...
            {
            w2 = f_w2_bin3_mips->Eval(trueBeamEnergy);
            w3 = f_w3_bin3_mips->Eval(trueBeamEnergy);
...
//          else if(hcal_bins==3)
//            {
//            w2 = f_w2_bin4_mips->Eval(trueBeamEnergy);
//            w3 = f_w3_bin4_mips->Eval(trueBeamEnergy);
//
//            pred = EE_detscale + w2*FH_detscale + w3*AH_detscale;
//            }
//
...
//          else
//            {
//            w2 = f_w2_bin5_mips->Eval(trueBeamEnergy);
//            w3 = f_w3_bin5_mips->Eval(trueBeamEnergy);
//
//            pred = EE_detscale + w2*FH_detscale + w3*AH_detscale;
//            }

  file_ecal_ene_frac<< ecal_ene_frac <<"\t"<< hcal_ene_frac <<"\t"<< trueBeamEnergy <<"\t"<< pred << "\t"<< hcal_bins+1<< "\t" << rechit_shower_start_layer<< "\t" << 0 <<"\n";

          }
          }//end of hcal_bins loop

      }

      else
      {
        for (int ecal_bins=0; ecal_bins<7; ecal_bins++)
        {//cout<<"ecal_bins="<<ecal_bins<<endl; 
          if(ecal_ene_frac>=ss_ecal_ene_frac_bins[ecal_bins] && ecal_ene_frac<ss_ecal_ene_frac_bins[ecal_bins+1])
          {//cout<<"ecal_ene_frac="<<ecal_ene_frac<<endl;
              for(int i_bin=0;i_bin<85;i_bin++)
              {
                  bool IsIn_en_range = true;
...
//            }
          ...
          file_ecal_ene_frac<< ecal_ene_frac <<"\t"<< hcal_ene_frac <<"\t"<< trueBeamEnergy <<"\t"<< pred << "\t"<< ecal_bins+1<< "\t" << rechit_shower_start_layer<< "\t" << 1 <<"\n";
  ...
  ////////////////////////////////////////////////////    
    ...
    for (int hcal_bins=0; hcal_bins<3; hcal_bins++)
    { 
      sprintf(name,"Results_new_chi2_7ss_3mips_bins/chi2_opt_mipsInEE_hcal_ene_fracFact_%f_to_%f_flatEn.txt",mips_hcal_ene_frac_bins[hcal_bins],mips_hcal_ene_frac_bins[hcal_bins+1]);
      ...
      for(int i_en =0; i_en<85; i_en++)
          ...
          bool isInverted = false;
              ...
              consts_mipsInEE = consts_mipsInEE_vec_diff_bins[hcal_bins][i_en];
              values_mipsInEE = values_mipsInEE_vec_diff_bins[hcal_bins][i_en];
              coeffs_mipsInEE = coeff_mipsInEE_vec_diff_bins[hcal_bins][i_en];

              isInverted = coeffs_mipsInEE.Invert();
              for(int i = 0; i <2; i++) {
                  for(int j = 0; j < 2; j++) {
                  cout<<coeffs_mipsInEE(i, j)<<"\t";
                  }
                  cout<<endl;
              }
              cout<<" "<<consts_mipsInEE(0)<<"\t"<<consts_mipsInEE(1)<<"\t"<<consts_mipsInEE(2)<<endl;
                                                                                                                              
              if(isInverted) {
                  values_mipsInEE = coeffs_mipsInEE*consts_mipsInEE;

                  cout<<"H Hadrons => For E = "<<Elist[i_en]+2<<"GeV, w1 = 0.0 ;w2 = "<<values_mipsInEE(0)<<" ;w3 = "<<values_mipsInEE(1)<<"\t"<<hcal_bins<<"\t"<<0<<endl;
                  cout<<">>>>>>> Uncertainity  : "<<sqrt(coeffs_mipsInEE(0,0)) << " " <<sqrt(coeffs_mipsInEE(1,1))<<endl;

                  file_bin1<<Elist[i_en]+2<<"\t"<<0<<"\t"<<values_mipsInEE(0)<<"\t"<<values_mipsInEE(1)<<"\t"<<0<<"\t"<<sqrt(coeffs_mipsInEE(0,0))<<"\t"<<sqrt(coeffs_mipsInEE(1,1))<<"\n";   
              
              }
              else {
                  cout<<"Error: Could not invert for mipsInEE_ene_frac hadrons..."<<endl;
                  cout<<"H Hadrons => For E = "<<Elist[i_en]+2<<"GeV, w1 = 0.0 ;w2 = "<<values_mipsInEE(0)<<" ;w3 = "<<values_mipsInEE(1)<<"\t"<<hcal_bins<<"\t"<<0<<endl;
              }
      ...
      for (int ecal_bins=0; ecal_bins<7; ecal_bins++)
      { 
      sprintf(name,"Results_new_chi2_7ss_3mips_bins/chi2_opt_ssInEE_ecal_ene_fracFact_%f_to_%f_flatEn.txt",ss_ecal_ene_frac_bins[ecal_bins],ss_ecal_ene_frac_bins[ecal_bins+1]);
      std::ofstream file_bin1;
      file_bin1.open(name,ios::out);
      ...
      for(int i_en =0; i_en<85; i_en++)
          ...
          bool isInverted = false;
              ...
              isInverted = coeffs_ssInEE.Invert();// cout<<"before invert"<<"\t"<<coeffs_ssInEE<<endl;
              for(int i = 0; i < 3; i++) {
                  for(int j = 0; j < 3; j++) {
