...
void AnalyzeHGCOctTB::EventLoop(const char *data, const char *energy) {
  ...
  sprintf(path_to_save, "Results_new_chi2_10ss_5mips_bins/");
 ...
 float ee_rescaling = 1.035;
  ...
  file_ecal_ene_frac.open("Results_new_chi2_10ss_5mips_bins/ecal_hcal_EneFrac_true_pred_SS.txt",ios::out);  
  ...
  sprintf(name1,"Results_new_chi2_10ss_5mips_bins/fitted_parameters_15bins.txt");
  std::fstream params;
...
//fit function SSInEE
  ...
  f_w3_bin10_ss->FixParameter(3, param_vector[179]); 
   ...
   for (int ecal_bin=0; ecal_bin<10; ecal_bin++){
     for(int i =0; i<85; i++)
         {
         for(int i = 0; i < 3; i++) {
             for(int j = 0; j < 3; j++) {
 ...
 //        cout<<ss_ecal_ene_frac_bins[ecal_bin]<<endl;
   ...
   for (int hcal_bin=0; hcal_bin<5; hcal_bin++){
     for(int i =0; i<85; i++)
       {
         for(int i = 0; i < 2; i++) {
           for(int j = 0; j < 2; j++) {
  ...
  for (jentry=0; jentry<nentries;jentry++,hgc_jentry++)
  {
    ...
    // ==============print number of events done == == == == == == == =
      ...
      {
        for (int hcal_bins=0; hcal_bins<5; hcal_bins++)
  ...
  file_ecal_ene_frac<< ecal_ene_frac <<"\t"<< hcal_ene_frac <<"\t"<< trueBeamEnergy <<"\t"<< pred << "\t"<< hcal_bins+1<< "\t" << rechit_shower_start_layer<< "\t" << 0 <<"\n";
      ...
      {
        for (int ecal_bins=0; ecal_bins<10; ecal_bins++)
        {//cout<<"ecal_bins="<<ecal_bins<<endl; 
  ...
  ////////////////////////////////////////////////////    
    ...
    for (int hcal_bins=0; hcal_bins<5; hcal_bins++)
    { 
      sprintf(name,"Results_new_chi2_10ss_5mips_bins/chi2_opt_mipsInEE_hcal_ene_fracFact_%f_to_%f_flatEn.txt",mips_hcal_ene_frac_bins[hcal_bins],mips_hcal_ene_frac_bins[hcal_bins+1]);
      ...
      for (int ecal_bins=0; ecal_bins<10; ecal_bins++)
      { 
      sprintf(name,"Results_new_chi2_10ss_5mips_bins/chi2_opt_ssInEE_ecal_ene_fracFact_%f_to_%f_flatEn.txt",ss_ecal_ene_frac_bins[ecal_bins],ss_ecal_ene_frac_bins[ecal_bins+1]);
      std::ofstream file_bin1;
      file_bin1.open(name,ios::out);
